...
exports.PathFnExpression = PathFnExpression;
...
function PathFnExpression(expression, afterSegments, meta) {
  this.expression = expression;
  this.afterSegments = afterSegments;
  this.meta = meta;
}
PathFnExpression.prototype = new Expression();
PathFnExpression.prototype.type = 'PathFnExpression';
PathFnExpression.prototype.serialize = function() {
  return serializeObject.instance(this, this.expression, this.afterSegments, this.meta);
};
PathFnExpression.prototype.get = function(context) {
  var path = this.expression.get(context);

  if (!path) return;

  var expression = this._createPathExpression(path);
...
PathFnExpression.prototype.resolve = function(context) {
  var path = this.expression.get(context);
  if (!path) return;
  var expression = this._createPathExpression(path);

  var base = expression.resolve(context);

  var segments = (this.afterSegments) ? base.concat(this.afterSegments) : base;
  return this._resolvePatch(context, segments);
};
PathFnExpression.prototype.dependencies = function(context, forInnerPath) {
  var path = this.expression.get(context);
  if (!path) return;
  var expression = this._createPathExpression(path);

  var expressionDeps = this.expression.dependencies(context);

  var outer = outerDependency(this, context, forInnerPath);

  return concat(outer, expressionDeps);
};
PathFnExpression.prototype.set = function(context, value) {
  var path = this.expression.get(context);
  if (!path) return;
  var expression = this._createPathExpression(path);
  return expression.set(context, value);
};
PathFnExpression.prototype._createPathExpression = function(path){
  ...
  if (firstChar === '#') {
    ...
    segments.shift();
    return new AliasPathExpression(alias, segments);
  } else if (firstChar === '@') {
    var attribute = firstSegment.slice(1);
    segments.shift();
    return new AttributePathExpression(attribute, segments);
  } else {
