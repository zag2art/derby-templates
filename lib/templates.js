...
exports.createEmptyTemplate = function() {
  return new saddle.Template([]);
};
...
function ParentWrapper(template, expression, isPartial) {
...
ParentWrapper.prototype.serialize = function() {
  return serializeObject.instance(this, this.template, this.expression, this.isPartial);
};
...
Views.prototype.serialize = function(options) {
  ...
  for (var name in this.nameMap) {
    ...
    // Notice that fullName is a js expression -- that's because we don't
    // know what the fullName is going to be. It's calculated on runtime
    // by prepending prefix when serialized view is being used with
    // 'namespace' argument.
